...
// Discover to get a device token + id. Returns 0 when paired; otherwise the
// number of seconds to sleep before retrying (device not yet claimed / error).
static int ensure_paired(const char *server) {
    ...
    char url[256];
    snprintf(url, sizeof(url), "%s/api/v1/device/discover", server);
    static char rbuf[768];
    rbuf[0] = '\0';
    resp_t r = { .body = rbuf, .cap = sizeof(rbuf) };
    int st = http_do(HTTP_METHOD_POST, url, NULL, NULL, NULL, body, &r);
    if (st != 200) { ESP_LOGW(TAG, "discover -> %d", st); return REST_DISCOVER_RETRY_S; }

    cJSON *j = cJSON_Parse(rbuf);
    if (!j) { ESP_LOGE(TAG, "discover response not JSON"); return REST_DISCOVER_RETRY_S; }
    ...
    if (cJSON_IsString(tok) && tok->valuestring[0]) {
        ...
        ESP_LOGI(TAG, "paired: device_id=%s", cJSON_IsString(id) ? id->valuestring : "?");
        retry = 0;
    } else {
