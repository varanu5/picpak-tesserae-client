...
Tesserae needs a renderer + device kind that produce/accept the PicPak's 2 bpp BWRY frame. Both are in
[`server-plugin/`](server-plugin/):

- `renderers/esp32_bwry_bin/` — Floyd-Steinberg dither → BWRY → pack 2 bpp / 30 000 B (MSB-first,
  vertical flip).
- `devices/picpak_client/` — the `picpak_client` device kind (400×300 panel).
